...
function getItemHref(locale: string, segment: string) {
  return segment ? `/${locale}/admin/${segment}` : `/${locale}/admin`;
}
...
function AdminNavList({closeOnClick = false}: AdminNavListProps) {
...
export function AdminSidebarMobileNav() {
  const t = useTranslations("adminDashboard");
