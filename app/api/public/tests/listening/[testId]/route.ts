...
export async function GET(_: Request, context: {params: Promise<{testId: string}>}) {
  ...
  const candidates = [
    `/api/v1/student/tests/listening/${encodeURIComponent(testId)}/`,
    `/api/v1/student/tests/${encodeURIComponent(testId)}/`,
    `/api/v1/public/tests/listening/${encodeURIComponent(testId)}/`,
  ...
  for (const endpoint of candidates) {
    ...
    try {
      ...
      return NextResponse.json({detail: "Failed to load listening test detail."}, {status: 503});
    }
  }
